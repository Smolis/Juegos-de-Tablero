...
public class Main {
	...
	final private static String DEFAULT_APP_MODE = "normal";
	 ...
	 * 
	 * <p>
	 * Numero de obstaculos proporcionadas con la opcion -o, o {@code null} si
	...
	private static void parseArgs(String[] args) {
		...
		cmdLineOptions.addOption(constructObsOption());
		cmdLineOptions.addOption(constructModeOption());
		// cmdLineOptions.addOption(construcServerPortOption());
	...
	private static Option constructModeOption() {
		return new Option("am", "app-mode", true, "Create the mode option"); // Hay
																				// que
																				// mejorar
																				// esta
																				// frase
	...
	private static void parseGameModeOptions(CommandLine line) {
		String am = null;
		if (line.getOptionValue("am") != null || line.getOptionValue("-app-mode") != null) {
			am = line.getOptionValue("am");
			applicationMode = am;
		} else {
			applicationMode = DEFAULT_APP_MODE;
		}
	}
	...
	// serverPort = Integer.parseInt(list.get(i));
	//
	// }
	// }
	//
	...
	private static Option constructMlutiViewOption() {
		return new Option("m", "multiviews", false, "Create a separate view for each player (valid only when using the " + ViewInfo.WINDOW + " view)");
	}
	...
	private static Option constructPlayersOption() {
		...
		optionInfo += "). If B is not given, the default mode '" + DEFAULT_PLAYERMODE.getId()
				+ "' is used. If this option is not given a default list of pieces from the corresponding game is used, each assigmed the mode '" + DEFAULT_PLAYERMODE.getId() + "'.";
	...
	private static Option constructObsOption() {
		return new Option("o", "obstacles", true, "The number of squares occupied by obstacles" + "(if allowed)");
	}

	/**
	 * Parses the obstacles option (-o). It sets the value of {@link #obstacles}
	 * .
	 * 
	 * <p>
	 * Extrae la opcion obstaculos (-o). Asigna el valor de los atributos
	 * {@link obstacles}.
	 * 
	 * @param line
	 *            CLI {@link CommandLine} object.
	...
	public static void startGameNoMVC() {
		...
		case CONSOLE:
			...
			break;
		case WINDOW:
			throw new UnsupportedOperationException("Swing Views are not supported in startGameNoMVC!! Please use startGameMVC instead.");
		default:
			throw new UnsupportedOperationException("Something went wrong! This program point should be unreachable!");
		}
	...
	private static void startServer() {
		...
		try {
			...
			throw new GameError("Error al iniciar server");
		}
	...
	private static void startClient() {
		try {
			...
			System.err.println("Error al iniciar cliente");
		}
	}
